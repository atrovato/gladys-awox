...
module.exports = function (device) {
  const macAddress = device.peripheral.address;
  const constructorName = (device.values['2a29'] || '').toString('utf-8').replace('\u0000', '').toLowerCase();

  if ('awox' != constructorName) {
    return Promise.reject(macAddress + ' is not an AwoX device');
  } else {
    return generateDevice(macAddress, device.values['2a24']).then((gladysDevice) => {
