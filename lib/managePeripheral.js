...
module.exports = function (peripheral, valueMap) {
  ...
  if ('awox' != constructorName) {
    ...
    return generateDevice(macAddress, valueMap.get('2a24')).then((device) => {
      return gladys.device.get(device.device).then((storedDevices) => {
        device.alreadyExists = storedDevices.length != 0;
...
function generateDevice(macAddress, deviceValue) {
  ...
  const remoteDevice = deviceModel.startsWith('rcum');
