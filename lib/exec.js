...
module.exports = function (deviceInfo) {
  return new Promise((resolve, reject) => {
    ...
    console.log('Built command for Awox ' + macAddr + ' (' + command + ')');
    ...
    return awoxScan(tmpPeripheral).then((peripherals) => {
      ...
      return awoxDiscoverServices(device);
    }).then((device) => {
      return awoxDiscoverCharacteristics(device);
    }).then((device) => {
      device.command = command;
      return awoxSend(device);
    }).then(() => {
      console.log('Awox command well done');
      resolve(value);
    });
  });
};
