...
module.exports = {
  ...
  decryptPacket: function (sessionKey, address, packet) {
    ...
    if (Buffer.from(check.slice(0, 1)).equals(packet.slice(5, 6))) {
      // Decrypted packet
      decrypted = Buffer.concat([packet.slice(0, 7), payload]);
  ...
  decodePacket: function (message) {
    ...
    if (messageType === 220) {
      const result = [];
      result['switch'] = message.readUIntBE(12, 1) % 2;
      result['white_temperature'] = message.readUIntBE(14, 1);
      result['white_brightness'] = message.readUIntBE(13, 1);
      result['color'] = message.readUIntBE(16, 3);
      result['color_brightness'] = message.readUIntBE(15, 1);
